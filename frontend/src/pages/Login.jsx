...
export default function Login() {
  ...
  async function submit(e) {
    e.preventDefault();
    try { await dispatch(login({ usernameOrEmail, password })).unwrap(); } catch {}
  }
    ...
    <div className="h-screen w-screen fixed inset-0 bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/80 text-lg font-body">Sign in to continue</p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 shadow-xl">
          <form onSubmit={submit} className="space-y-5">
            <div>
              <input 
                className="w-full px-4 py-3 bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl text-white placeholder-white/60 focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400/50 transition-all duration-300" 
            ...
            {error && (
              <div className="bg-red-500/20 border border-red-400/40 rounded-xl p-3 backdrop-blur-lg">
        ...
        <div className="text-center mt-8">
          <p className="text-white/50 text-sm">© 2024 EduConnect. All rights reserved.</p>
        </div>
      </div>
    </div>
  );
